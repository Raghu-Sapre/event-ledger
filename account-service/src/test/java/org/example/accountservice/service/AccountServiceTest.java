...
class AccountServiceTest {
    ...
    void getAccount_WhenExists_ShouldReturnAccount() {
        Account expectedAccount = Account.builder().accountId(accountId).balance(BigDecimal.TEN).build();
    ...
    void applyEvent_WhenDuplicateEventId_ShouldReturnIdempotentState() {
        ApplyEventRequest duplicateRequest = new ApplyEventRequest(
                "evt-dup", accountId, new BigDecimal("50.00"), "CREDIT", Instant.now()
        );
        Account existingAccount = Account.builder().accountId(accountId).balance(new BigDecimal("100.00")).build();
        ...
        // Verify balance was untouched, and no new transaction or account was saved
    ...
    void applyEvent_WithOutOfOrderEvents_ShouldRecalculateChronologicalCorrectBalance() {
        ...
        ApplyEventRequest lateArrivingEvent = new ApplyEventRequest(
                "evt-late", accountId, new BigDecimal("30.00"), "DEBIT", time1
        );

        Account account = Account.builder().accountId(accountId).balance(new BigDecimal("100.00")).build();

        // Mock database transactions timeline representing history already containing the later event
        AccountTransaction existingLaterTx = AccountTransaction.builder()
                .eventId("evt-future").accountId(accountId).amount(new BigDecimal("100.00")).type("CREDIT").eventTimestamp(time2)
                .build();

        AccountTransaction newLateTx = AccountTransaction.builder()
                .eventId("evt-late").accountId(accountId).amount(new BigDecimal("30.00")).type("DEBIT").eventTimestamp(time1)
                .build();
        ...
        // Critically: The repository returns them sorted chronologically by timestamp (Time1, then Time2)
        List<AccountTransaction> chronologicallySortedList = Arrays.asList(newLateTx, existingLaterTx);
        when(transactionRepository.findByAccountIdOrderByEventTimestampAsc(accountId)).thenReturn(chronologicallySortedList);
